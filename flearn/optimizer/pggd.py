...
class PerGodGradientDescent(optimizer.Optimizer):
    ...
    def _apply_dense(self, grad, var):
        ...
        var_update = state_ops.assign_sub(var, lr_t * (
                    grad + gold + mu_t * (var - vstar)))
    ...
    def set_params(self, cog, avg_gradient, client):
        ...
        # get old gradient
        prev = client.get_grads()

        # Find g_t - F'(old)
        diff = [g1 - g2 for g1, g2 in zip(avg_gradient, prev)]

        with client.model.graph.as_default():
            all_vars = tf.compat.v1.trainable_variables()
            for variable, grad in zip(all_vars, diff):

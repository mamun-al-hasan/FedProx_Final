...
for log in ["synthetic_iid", "synthetic_0_0", "synthetic_0.5_0.5", "synthetic_1_1"]:
    ...
    if sys.argv[1] == 'loss':
        plt.plot(np.asarray(rounds1[:len(losses1)]), np.asarray(losses1), '--', linewidth=3.0, label='mu=0, E=20',
                 color="#17becf")
        plt.plot(np.asarray(rounds2[:len(losses2)]), np.asarray(losses2), linewidth=3.0, label='mu=1, E=20',
                 color="#e377c2")

    elif sys.argv[1] == 'accuracy':
        plt.plot(np.asarray(rounds1[:len(test_accuracies1)]), np.asarray(test_accuracies1), '--', linewidth=3.0,
                 label='mu=0, E=20', color="#17becf")
        plt.plot(np.asarray(rounds2[:len(test_accuracies2)]), np.asarray(test_accuracies2), linewidth=3.0,
                 label='mu=1, E=20', color="#e377c2")
    else:
        plt.plot(np.asarray(rounds1[:len(sim1)]), np.asarray(sim1), '--', linewidth=3.0, label='mu=0, E=20',
                 color="#17becf")
